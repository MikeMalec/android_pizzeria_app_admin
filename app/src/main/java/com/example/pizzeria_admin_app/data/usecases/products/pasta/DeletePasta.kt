...
@Singleton
class DeletePasta @Inject constructor(val productRemoteDataSource: ProductRemoteDataSource) {
    fun deletePasta(token:String,id: Int): Flow<Resource<GenericResponse?>> = flow {
        emit(Resource.Loading())
        val response = safeApiCall(Dispatchers.IO) { productRemoteDataSource.deletePasta(token,id) }
        emit(response)
    }
}