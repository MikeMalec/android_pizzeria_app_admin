...
@Singleton
class DeleteSalad @Inject constructor(val productRemoteDataSource: ProductRemoteDataSource) {
    fun deleteSalad(token: String, id: Int): Flow<Resource<GenericResponse?>> = flow {
        emit(Resource.Loading())
        val response =
            safeApiCall(Dispatchers.IO) { productRemoteDataSource.deleteSalad(token, id) }
        emit(response)
    }
}