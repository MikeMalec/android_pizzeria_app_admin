...
@Singleton
class UpdateBurger @Inject constructor(val productRemoteDataSource: ProductRemoteDataSource) {
    fun updateBurger(token:String,id: Int, burger: Burger): Flow<Resource<GenericResponse?>> = flow {
        emit(Resource.Loading())
        val response =
            safeApiCall(Dispatchers.IO) { productRemoteDataSource.updateBurger(token,id, burger) }
        emit(response)
    }
}