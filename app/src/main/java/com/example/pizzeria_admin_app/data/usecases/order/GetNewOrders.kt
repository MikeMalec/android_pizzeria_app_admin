...
@Singleton
class GetNewOrders @Inject constructor(val orderRemoteDataSource: OrderRemoteDataSource) {
    fun getNewOrders(token: String): Flow<Resource<OrdersResponse?>> = flow {
        emit(Loading())
        val response = safeApiCall(IO) {
            orderRemoteDataSource.getNewOrders(token)
        }
        emit(response)
    }
}